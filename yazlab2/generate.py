...
# Etkinlik sınıfı
class Event:
    def __init__(self, name, description, date, time, duration, location, category, image_url, il, olusturanid):
        self.name = name
    ...
    def save(self):
        cursor.execute("""
            INSERT INTO events (name, description, date, time, duration, location, category, image_url, il, olusturanid)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (self.name, self.description, self.date, self.time, self.duration, self.location, self.category, self.image_url, self.il, self.olusturanid))
        db.commit()
